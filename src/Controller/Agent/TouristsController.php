...
final class TouristsController
{
    ...
    public function update(Request $request, Response $response, array $args): Response
    {
        ...
        if (stripos($accept, 'application/json') !== false || $request->getHeaderLine('X-Requested-With') === 'XMLHttpRequest') {
            $response->getBody()->write(json_encode(['ok'=>true,'message'=>'Данные туриста сохранены'], JSON_UNESCAPED_UNICODE));
            return $response->withHeader('Content-Type','application/json');
        }        
    ...
    public function upload(Request $request, Response $response, array $args): Response
    {
        ...
        $accept = $request->getHeaderLine('Accept');
        if (stripos($accept, 'application/json') !== false || $request->getHeaderLine('X-Requested-With') === 'XMLHttpRequest') {
            $response->getBody()->write(json_encode(['ok'=>true,'message'=>'Документы туриста закреплены'], JSON_UNESCAPED_UNICODE));
            return $response->withHeader('Content-Type','application/json');
        }           
        return $response->withHeader('Location','/agent/tourists/'.$id.'/edit')->withStatus(302);
    }
    ...
    public function deleteFile(Request $request, Response $response, array $args): Response
    {
        ...
        $accept = $request->getHeaderLine('Accept');
        if (stripos($accept, 'application/json') !== false || $request->getHeaderLine('X-Requested-With') === 'XMLHttpRequest') {
            $response->getBody()->write(json_encode(['ok'=>true,'message'=>'Документы удален'], JSON_UNESCAPED_UNICODE));
            return $response->withHeader('Content-Type','application/json');
        }          
        return $response->withHeader('Location','/agent/tourists/'.$id.'/edit')->withStatus(302);
    }
